...
class TokenStoreTest(unittest.TestCase):
    ...
    def test_token_fits_in_callback_data(self):
        token = bot.tok("light.salone_principale")
    ...
    def test_round_trip(self):
        ...
        self.assertEqual(bot.untok(bot.tok(value)), value)

    def test_same_value_reuses_the_same_token(self):
        self.assertEqual(bot.tok("Salone"), bot.tok("Salone"))
        self.assertEqual(len(bot._tokens), 1)

    def test_unknown_token_is_none_not_an_error(self):
        self.assertIsNone(bot.untok("deadbeef1234"))

    def test_store_is_bounded(self):
        with mock.patch.object(bot, "MAX_TOKENS", 3):
            for i in range(10):
                bot.tok(f"value-{i}")
            self.assertEqual(len(bot._tokens), 3)

    def test_eviction_drops_the_oldest(self):
        with mock.patch.object(bot, "MAX_TOKENS", 3):
            a, b, c = (bot.tok(v) for v in ("a", "b", "c"))
            bot.tok("d")
            self.assertIsNone(bot.untok(a))
            self.assertEqual(bot.untok(b), "b")

    def test_use_refreshes_the_position(self):
        """A keyboard still in use must not expire under a busy one."""
        with mock.patch.object(bot, "MAX_TOKENS", 3):
            a, b, c = (bot.tok(v) for v in ("a", "b", "c"))
            bot.untok(a)          # a is now the most recently used
            bot.tok("d")          # evicts b, not a
            self.assertEqual(bot.untok(a), "a")
            self.assertIsNone(bot.untok(b))
...
class AreaMatchingTest(unittest.TestCase):
    ...
    def test_area_name_translates_only_the_sentinel(self):
        self.assertEqual(self.b._area_name("Salone", "en"), "Salone")
        self.assertEqual(self.b._area_name(ent.NO_AREA, "it"), "Senza stanza")
        self.assertEqual(self.b._area_name(ent.NO_AREA, "en"), "No room")
...
class RenderingTest(unittest.TestCase):
    ...
    def test_areas_summary_counts_unreachable_as_off(self):
        text = self.b._areas_summary(self.lights, AREAS, "it")
    ...
    def test_areas_summary_localises_the_no_area_bucket(self):
        self.assertIn("Senza stanza", self.b._areas_summary(self.lights, AREAS, "it"))
        self.assertIn("No room", self.b._areas_summary(self.lights, AREAS, "en"))

    def test_lights_text_escapes_names(self):
        text = self.b._lights_text("Terrazza", self.lights, "it")
        self.assertIn("Luce &lt;terrazza&gt; &amp; co", text)
        self.assertNotIn("<terrazza>", text)

    def test_lights_text_escapes_the_title(self):
        self.assertIn("&lt;script&gt;", self.b._lights_text("<script>", [], "it"))
    ...
    def test_lights_keyboard_offers_the_opposite_action(self):
        ...
        data = payloads(self.b._lights_keyboard(lights, "it"))
    ...
    def test_lights_keyboard_ends_with_bulk_and_refresh(self):
        kb = self.b._lights_keyboard(self.lights, "en")
    ...
    def test_bulk_buttons_share_one_token_with_everything_displayed(self):
        kb = self.b._lights_keyboard(self.lights, "it")
        data = payloads(kb)
        on_token = data[-3].split(":")[-1]
        self.assertEqual(bot.untok(on_token).split("|"), [s["entity_id"] for s in self.lights])

    def test_lights_keyboard_is_capped(self):
        with mock.patch.object(bot, "MAX_BUTTONS", 2):
            kb = self.b._lights_keyboard(self.lights, "it")
...
class AudioFormatTest(unittest.TestCase):
    def test_wav_is_declared_as_pcm(self):
        self.assertEqual(bot.HassBot._audio_format("audio/wav"), ("wav", "pcm"))
        self.assertEqual(bot.HassBot._audio_format("audio/x-wav"), ("wav", "pcm"))

    def test_everything_else_is_ogg_opus(self):
        # what Telegram voice notes are, and what HA's providers accept natively
        self.assertEqual(bot.HassBot._audio_format("audio/ogg"), ("ogg", "opus"))
        self.assertEqual(bot.HassBot._audio_format(None), ("ogg", "opus"))
        self.assertEqual(bot.HassBot._audio_format("video/mp4"), ("ogg", "opus"))
